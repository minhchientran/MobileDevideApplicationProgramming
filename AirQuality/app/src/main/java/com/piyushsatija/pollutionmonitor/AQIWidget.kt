        ...
        fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager,
            ...
            when {
                aqi in 0..50 -> {
                    ...
                    views.setTextColor(R.id.widget_air_quality_text, ContextCompat.getColor(context, R.color.scaleGood))

                }
                aqi in 51..100 -> {
                    airQuality = context.getString(R.string.moderate)
                    ContextCompat.getColor(context, R.color.scaleModerate)
                    views.setTextColor(R.id.widget_air_quality_text, ContextCompat.getColor(context, R.color.scaleModerate))
                }
                aqi in 101..150 -> {
                    airQuality = context.getString(R.string.unhealthy)
                    ContextCompat.getColor(context, R.color.scaleUnhealthySensitive)
                    views.setTextColor(R.id.widget_air_quality_text, ContextCompat.getColor(context, R.color.scaleUnhealthySensitive))
                }
                aqi in 151..200 -> {
                    airQuality = context.getString(R.string.unhealthy)
                    ContextCompat.getColor(context, R.color.scaleUnhealthy)
                    views.setTextColor(R.id.widget_air_quality_text, ContextCompat.getColor(context, R.color.scaleUnhealthy))
                }
                aqi in 201..300 -> {
                    airQuality = context.getString(R.string.very_unhealthy)
                    ContextCompat.getColor(context, R.color.scaleVeryUnhealthy)
                    views.setTextColor(R.id.widget_air_quality_text, ContextCompat.getColor(context, R.color.scaleVeryUnhealthy))
                }
                aqi >= 301 -> {
                    airQuality = context.getString(R.string.hazardous)
                    ContextCompat.getColor(context, R.color.scaleHazardous)
                    views.setTextColor(R.id.widget_air_quality_text, ContextCompat.getColor(context, R.color.scaleHazardous))
                }
                else -> {
                    ContextCompat.getColor(context, R.color.scaleGood)
                    views.setTextColor(R.id.widget_air_quality_text, ContextCompat.getColor(context, R.color.scaleGood))
